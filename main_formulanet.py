...
from model.resnet.model_resnet import FormulaNet as Model
...
def init_parse():
    ...
    parser.add_argument('--output', type=str, default='runs/formulanet/3view_v11nfci_ori', help='output directory')
    ...
    parser.add_argument('--data_path', type=str, default='/home/user/dataset',
                        help='data path')
    parser.add_argument('--data', type=str, default='main_auxiliary',
                        choices=['ori', 'yolo_fci', 'main_auxiliary'],
                        help='ori means Original Images, fci represents Foreground Consistent image')
    parser.add_argument('--viewpoint', type=str, default='formulanet', 
                        choices=['mv', 'sv', 'mv3', 'mv3_cat', 'formulanet'],
                        help='sv means Single View, while mv means Multi View')
...
def evaluate(net, data_loader, criterion, device, viewpoint):
    ...
    with torch.no_grad():
        for data in tqdm(data_loader, desc="Evaluating"):
            if viewpoint in ['sv', 'mv3_cat']:
                ...
                outputs = net(view)
            elif viewpoint in ['mv', 'formulanet']:
    ...
    for id in id_correct_count:
        ...
        # If all samples corresponding to the ID are predicted correctly
...
if __name__ == '__main__':
    ...
    elif args.viewpoint == 'formulanet':  
        net = Model().to(device)
        ...
        for epoch in range(args.num_epochs):
            ...
            for itern, data in enumerate(pbar):
                if args.viewpoint in ['sv', 'mv3_cat']:
                    ...
                    labels = data[1].to(device)
                    outputs = net(view)
                elif args.viewpoint in ['mv', 'formulanet']:
                    view1 = data[0].to(device)
                    view2 = data[1].to(device)
                    labels = data[2].to(device)
                    outputs = net(view1, view2)
